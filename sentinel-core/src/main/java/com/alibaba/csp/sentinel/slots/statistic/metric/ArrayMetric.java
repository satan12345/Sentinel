...
public class ArrayMetric implements Metric {
    ...
    @Override
    public long pass() {
        //更新array中当前时间点所在的样本窗口实例的数据
        data.currentWindow();
        long pass = 0;
        //将当前时间创中所有的样布统计的value 记录到result中
        List<MetricBucket> list = data.values();        //遍历 将pass维度的数据取出求和
        for (MetricBucket window : list) {
            pass += window.pass();
        }
        return pass;
