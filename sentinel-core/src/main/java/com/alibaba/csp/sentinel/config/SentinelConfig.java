...
/**
 * The universal local config center of Sentinel. The config is retrieved from command line
 * arguments and {@code ${user.home}/logs/csp/${appName}.properties} file by default.
 *
 * @author leyou
 * @author Eric Zhao
 */
public class SentinelConfig {
  ...
  private static void loadProps() {
    ...
    for (Map.Entry<Object, Object> entry : new CopyOnWriteArraySet<>(
      ...
      if (configValueOld != null) {
        RecordLog.info("[SentinelConfig] JVM parameter overrides {0}: {1} -> {2}", configKey,
            configValueOld, configValue);
      }
    }
  }
  ...
  public static int coldFactor() {
    try {
      ...
      if (coldFactor <= 1) {
        ...
        RecordLog
            .warn("cold factor=" + coldFactor + ", should be larger than 1, use default value: "
                + DEFAULT_COLD_FACTOR);
      }
      return coldFactor;
    } catch (Throwable throwable) {
